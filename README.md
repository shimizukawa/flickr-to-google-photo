...
Google Photosへのアップロード前に、同じ写真がすでにライブラリに存在しないかを自動的に確認します。

### いつ検出するか

アップロードステップ（`uploading`）の直前に毎回チェックします。具体的には、Flickrの `date_taken`（撮影日）を使ってその日付の写真をGoogle Photos APIで検索し、以下の3つの戦略で一致判定します。`date_taken` が不明な写真はチェックをスキップします。

### 3つの検出戦略（優先順）

| 優先度 | 戦略 | 説明 |
|--------|------|------|
| 1 | **ファイル名一致** | Google Photos上の `filename` がダウンロードしたファイル名と完全一致。このアプリが過去にアップロードした写真の再実行時に確実に検出できます。 |
| 2 | **タイムスタンプ一致（分:秒）** | 撮影時刻の「分」と「秒」が一致するかどうかを比較。FlickrはローカルタイムでGoogle PhotosはUTCで保存するため時単位でずれますが、タイムゾーンオフセットは常に整数時間なので分・秒は一致します。スマートフォンからGoogle Photosへ直接アップロードした写真など、このアプリ以外で登録した写真も検出できます。 |
| 3 | **寸法一致（幅×高さ）** | ピクセル寸法が一致するかどうかを確認。`creationTime` が取得できない場合の補助的な戦略です。同一機種で撮影した別の写真と誤検出される可能性があります。 |

### 検出時の動作

重複が検出された場合、**アップロードをスキップ**し、既存のメディアアイテムIDを `google_photo_id` として記録してステータスを `uploaded` に進めます。その後、アルバムへの追加（`adding_to_album`）は通常通り実行されます。

```
重複なし: DOWNLOADED → UPLOADING → UPLOADED → ADDING_TO_ALBUM → COMPLETED
重複あり: DOWNLOADED → UPLOADED（スキップ）  → ADDING_TO_ALBUM → COMPLETED
```

### スマートフォン写真への対応

Google Photos APIの `photoslibrary.readonly` スコープを使用することで、このアプリ以外でアップロードされた写真（スマートフォンの自動バックアップ等）も検索対象に含まれます。初回認証時にこのスコープへの許可が求められます。
